...
def main():
    ...
    exp_name = "HousePricePrediction3"
    ...
    with mlflow.start_run(run_name="parent_run") as parent_run:
        ...
        with mlflow.start_run(run_name="Ingest_Data", nested=True) as ingest_data_child_run:
            print("Running ingest.py")
            run_script(ingest_data_run, ingest_data_child_run.info.run_id)

        with mlflow.start_run(run_name="train_Data",nested=True) as train_child_run:
            print("Running script_train.py")
            run_script(train_run, train_child_run.info.run_id)

        with mlflow.start_run(run_name="score_Data",nested=True) as score_child_run:
